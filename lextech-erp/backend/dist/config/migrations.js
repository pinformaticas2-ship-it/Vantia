...
async function runMigrations() {
    ...
    `);
        try {
            await client.query(`CREATE INDEX IF NOT EXISTS idx_quipu_settings_user_id ON quipu_settings (user_id);`);
        }
        catch (_e) { }
        try {
            await client.query(`DROP TRIGGER IF EXISTS trg_quipu_settings_updated_at ON quipu_settings;`);
        }
        catch (_e) { }
        try {
            await client.query(`
