...
exports.chatVantia = exports.getChatHistory = void 0;
const database_1 = __importDefault(require("../config/database"));
const SYSTEM_PROMPT = 'Eres VantIA, el asistente inteligente de VANTIA Legis ERP, ' +
...
const getChatHistory = async (req, res) => {
    ...
    try {
        const result = await database_1.default.query('SELECT history FROM vantia_chat_history WHERE user_id = $1 AND module_id = $2', [userId, String(moduleId)]);
...
const chatVantia = async (req, res) => {
    const { message, history = [], systemPrompt, moduleId, } = req.body;
    const userId = req.auth?.userId;
    if (!message?.trim()) {
    ...
    try {
        ...
        if (moduleId.startsWith('/dashboard/clientes')) {
            try {
                const recentClients = await database_1.default.query(`SELECT first_name, last_name, commercial_name 
           FROM entities 
           WHERE type = 'CLIENTE' 
        ...
        DO UPDATE SET
          ...
          updated_at = NOW();
      `;
            await database_1.default.query(upsertQuery, [userId, moduleId, JSON.stringify(newHistory)]);
        }
        catch (dbError) {
