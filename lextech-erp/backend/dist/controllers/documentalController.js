...
exports.getDocumentalProviders = getDocumentalProviders;
exports.getBoeSchemas = getBoeSchemas;
exports.searchBoeDocuments = searchBoeDocuments;
exports.getBoeDocumentById = getBoeDocumentById;
exports.getBoeBlockById = getBoeBlockById;
exports.getCendojHighlights = getCendojHighlights;
exports.searchCendoj = searchCendoj;
...
const BOE_SCHEMA_DIR = path_1.default.resolve(__dirname, '../../resources/boe');
...
function normalizeLooseMultilineText(text) {
    return decodeHtmlText(text)
        .replace(/\r/g, '')
        .replace(/[ \t]+\n/g, '\n')
        .replace(/\n{3,}/g, '\n\n')
...
function extractBoeBlockContent(xml) {
...
function extractCendojHighlights(html) {
...
function buildCendojSearchUrl(query, page = 1) {
...
function htmlToReadableText(html) {
    return normalizeLooseMultilineText(html
        .replace(/\r/g, '')
...
function extractLineValue(block, labels) {
...
function sanitizeCendojSummary(text) {
    if (!text)
        return null;
    return text
...
function dedupeCendojResults(items) {
    const seen = new Set();
...
function normalizeCendojKey(value) {
    return normalizeSearchableText(value).replace(/\s+/g, '');
}
function extractCendojSearchResults(html) {
    ...
    }).filter((item) => Boolean(item?.roj || item?.ecli));
    return {
...
function getLocalBoeSchemaSummary() {
    const files = ['sumario-boe.xsd', 'tipos.xsd']
        .map((name) => {
        const fullPath = path_1.default.join(BOE_SCHEMA_DIR, name);
...
function normalizeBoeSearchTerm(value) {
    return value.trim().replace(/\s+/g, ' ');
}
function escapeBoeQueryValue(value) {
...
function buildBoeQueryString(parts) {
...
function scoreBoeItem(item, filters) {
    ...
    const materias = normalizeSearchableText(Array.isArray(item.materias)
        ? item.materias.map((entry) => entry?.texto || entry).join(' ')
        : item.materias);
    ...
    if (query) {
        ...
        if (titleText.includes(query))
            score += 90;
        if (numero.includes(query))
            score += 110;
        if (materias.includes(query))
            score += 40;
        if (departamento.includes(query))
            score += 20;
    ...
    if (filters.title) {
        ...
        if (titleText === exactTitle)
            score += 120;
        if (titleText.includes(exactTitle))
            score += 60;
    }
    if (filters.rango && rango.includes(normalizeSearchableText(filters.rango)))
        score += 25;
    if (filters.departamento && departamento.includes(normalizeSearchableText(filters.departamento)))
        score += 25;
    if (filters.materia && materias.includes(normalizeSearchableText(filters.materia)))
        score += 25;
    if (filters.texto)
        score += 5;
    return score;
}
function dedupeAndRankBoeItems(items, filters) {
    const seen = new Set();
    return items
        .map((item) => ({
        raw: item,
...
async function getDocumentalProviders(_req, res) {
...
async function getBoeSchemas(_req, res) {
    return ok(res, getLocalBoeSchemaSummary());
}
async function searchBoeDocuments(req, res) {
...
async function getBoeDocumentById(req, res) {
...
async function getBoeBlockById(req, res) {
    const id = String(req.params.id || '').trim().toUpperCase();
    ...
    try {
        ...
        if (!parsed)
            return err(res, 'No se pudo interpretar el bloque del BOE.', 502);
        return ok(res, {
            documentId: id,
            ...parsed,
...
async function getCendojHighlights(_req, res) {
    try {
...
async function searchCendoj(req, res) {
