...
exports.syncGoogleEvents = exports.importGoogleEvents = exports.deleteEvent = exports.updateEvent = exports.createEvent = exports.getEventById = exports.getOrganizationOptions = exports.getUpcomingEvents = exports.getEvents = void 0;
...
const agendaStatusToTaskEstado = (status) => status === 'completado' ? 'completada' : 'pendiente';
const agendaTypeToTaskTipo = (type) => {
    if (type === 'reunion')
        return 'reunion';
    if (type === 'vista')
        return 'vista_juicio';
...
const agendaEventToTaskDate = (startAt) => startAt ? new Date(startAt).toISOString().slice(0, 10) : null;
const shouldSyncAgendaEventToTask = (event) => event.type === 'plazo' && Boolean(event.cliente_id);
...
const normalizeGoogleEvent = (raw) => {
...
const getEvents = async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`SELECT * FROM agenda_events ${where} ORDER BY start_at ASC LIMIT 500`, vals);
...
const getUpcomingEvents = async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`SELECT * FROM agenda_events
...
const getOrganizationOptions = async (_req, res) => {
    try {
        const [expedientesRes, linksRes] = await Promise.all([
         ...
         LIMIT 500`),
            database_1.default.query(`SELECT DISTINCT expediente_id, user_id, user_name
         FROM (
        ...
        const linksByExpediente = new Map();
...
exports.getOrganizationOptions = getOrganizationOptions;
const getEventById = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await database_1.default.query(`SELECT * FROM agenda_events WHERE id = $1`, [id]);
...
const createEvent = async (req, res) => {
    ...
    const { title, description, start_at, end_at, all_day, type, status, expediente_id, cliente_id, related_user_id, related_user_name, organization_context, location, color, source, external_provider, external_id, external_url, } = req.body;
    ...
    const { resolveUserName } = await Promise.resolve().then(() => __importStar(require('./activityController')));
    const userName = userId === 'SYSTEM' ? 'Sistema' : await resolveUserName(userId);
    try {
        const result = await database_1.default.query(`INSERT INTO agenda_events
       ...
       RETURNING *`, [
        ...
        if (shouldSyncAgendaEventToTask(createdEvent)) {
            const clientRow = await database_1.default.query(`SELECT COALESCE(commercial_name, first_name || ' ' || COALESCE(last_name,'')) AS name FROM entities WHERE id = $1`, [createdEvent.cliente_id]);
            const clientName = clientRow.rows[0]?.name || null;
            const taskDate = agendaEventToTaskDate(createdEvent.start_at);
            const taskRes = await database_1.default.query(`INSERT INTO client_tasks
         ...
         RETURNING *`, [
            ...
            await database_1.default.query(`UPDATE agenda_events SET task_id = $1, updated_at = NOW() WHERE id = $2`, [createdEvent.task_id, createdEvent.id]);
        }
        await (0, activityController_1.logActivityForReq)(req, 'Evento agenda creado', 'AGENDA', createdEvent.id, sanitizeTitle(title));
...
const updateEvent = async (req, res) => {
    ...
    try {
        const existingQ = await database_1.default.query(`SELECT * FROM agenda_events WHERE id = $1`, [id]);
        if (existingQ.rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Evento no encontrado' });
        }
        const existingEvent = existingQ.rows[0];
        const result = await database_1.default.query(`UPDATE agenda_events SET
         title = $1,
         description = $2,
         start_at = $3,
         end_at = $4,
       ...
       RETURNING *`, [
        ...
        if (shouldSyncAgendaEventToTask(updatedEvent) && updatedEvent.task_id) {
            ...
            await database_1.default.query(`UPDATE client_tasks
         SET titulo = $1,
             descripcion = $2,
             plazo = $3,
             fecha_aviso = $4,
         ...
         WHERE id = $10`, [
            ...
            const clientRow = await database_1.default.query(`SELECT COALESCE(commercial_name, first_name || ' ' || COALESCE(last_name,'')) AS name FROM entities WHERE id = $1`, [updatedEvent.cliente_id]);
            const clientName = clientRow.rows[0]?.name || null;
            const taskDate = agendaEventToTaskDate(updatedEvent.start_at);
            const taskRes = await database_1.default.query(`INSERT INTO client_tasks
           (client_id, client_name, titulo, descripcion, plazo, fecha_aviso, estado, prioridad,
         ...
         RETURNING *`, [
            ...
            await database_1.default.query(`UPDATE agenda_events SET task_id = $1, updated_at = NOW() WHERE id = $2`, [updatedEvent.task_id, updatedEvent.id]);
        }
        else if (!shouldSyncAgendaEventToTask(updatedEvent) && existingEvent.task_id) {
            await database_1.default.query(`DELETE FROM client_tasks WHERE id = $1`, [existingEvent.task_id]);
            await database_1.default.query(`UPDATE agenda_events SET task_id = NULL, updated_at = NOW() WHERE id = $1`, [updatedEvent.id]);
            updatedEvent.task_id = null;
        }
        await (0, activityController_1.logActivityForReq)(req, 'Evento agenda modificado', 'AGENDA', id, sanitizeTitle(title));
...
const deleteEvent = async (req, res) => {
    const { id } = req.params;
    try {
        const existing = await database_1.default.query(`SELECT title, task_id FROM agenda_events WHERE id = $1`, [id]);
        if (existing.rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Evento no encontrado' });
        }
        if (existing.rows[0].task_id) {
            await database_1.default.query(`DELETE FROM client_tasks WHERE id = $1`, [existing.rows[0].task_id]);
        }
        await database_1.default.query(`DELETE FROM agenda_events WHERE id = $1`, [id]);
        await (0, activityController_1.logActivityForReq)(req, 'Evento agenda eliminado', 'AGENDA', id, existing.rows[0].title);
...
const importGoogleEvents = async (req, res) => {
    ...
    for (const raw of rawEvents) {
        ...
        try {
            const existing = await database_1.default.query(`SELECT * FROM agenda_events
         WHERE external_provider = 'google' AND external_id = $1
         LIMIT 1`, [event.externalId]);
            if (existing.rows.length > 0) {
         ...
         RETURNING *`, [
            ...
            await (0, activityController_1.logActivityForReq)(req, 'Evento Google importado a agenda', 'AGENDA', result.rows[0].id, event.title);
        }
        catch (error) {
            errors.push({
                external_id: event.externalId,
...
exports.importGoogleEvents = importGoogleEvents;
const syncGoogleEvents = async (req, res) => {
    ...
    try {
        const existingInRange = await database_1.default.query(`SELECT id, external_id, title
       FROM agenda_events
       WHERE external_provider = 'google'
         AND start_at >= $1
         AND start_at <= $2`, [from, to]);
        for (const row of existingInRange.rows) {
            if (row.external_id && !seenIds.has(row.external_id)) {
                await database_1.default.query(`DELETE FROM agenda_events WHERE id = $1`, [row.id]);
                deleted.push(row);
            }
        }
    }
