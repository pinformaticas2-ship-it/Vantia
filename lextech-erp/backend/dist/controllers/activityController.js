...
function getClerk() {
    if (!_clerk)
        _clerk = (0, backend_1.createClerkClient)({ secretKey: process.env.CLERK_SECRET_KEY });
    return _clerk;
}
...
async function logActivity(userId, userName, actionType, entityType, entityId, entityName, opts) {
    try {
        await database_1.default.query(`INSERT INTO activity_log
...
async function logActivityForReq(req, actionType, entityType, entityId, entityName, eventType) {
...
const registerLogin = async (req, res) => {
    ...
    try {
      ...
      ORDER BY created_at DESC LIMIT 1`, [userId, ip, deviceId]);
        if (rows.length) {
            return res.json({ success: true, data: { already: true } });
        }
        await logActivity(userId, userName, `Inicio de sesión desde ${ip}`, undefined, undefined, undefined, { eventType: 'LOGIN', ipAddress: ip, sessionId, userAgent: getUA(req), deviceId });
...
const getClientActivity = async (req, res) => {
    ...
    try {
      ...
      ORDER BY al.created_at DESC LIMIT 200`, [clientId]);
        res.json({ success: true, data: rows });
    }
    catch (e) {
        res.status(500).json({ success: false, error: pgErr(e) });
...
const addClientActivity = async (req, res) => {
    ...
    try {
        const cr = await database_1.default.query(`SELECT COALESCE(commercial_name,CONCAT(first_name,' ',last_name)) AS name FROM entities WHERE id=$1`, [clientId]);
        const entityName = cr.rows[0]?.name || clientId;
        const fullAction = description?.trim()
            ? `${action_type.trim()}: ${description.trim()}`
            : action_type.trim();
        const { rows } = await database_1.default.query(`INSERT INTO activity_log
         (user_id,user_name,action_type,entity_type,entity_id,entity_name,event_type,ip_address,user_agent,device_id)
       VALUES ($1,$2,$3,'CLIENT',$4,$5,'ACTION',$6,$7,$8) RETURNING *`, [userId, userName, fullAction, clientId, entityName, (0, clientIp_1.getClientIp)(req), getUA(req), getDeviceId(req)]);
...
const getActivityByUsers = async (_req, res) => {
    try {
        const { rows } = await database_1.default.query(`
      SELECT
        user_id,
...
exports.getActivityByUsers = getActivityByUsers;
