...
exports.downloadBlank = exports.downloadTemplate = exports.previewTemplateAsPdf = exports.listTemplates = exports.previewTemplateAsHtml = exports.UPLOADS_ROOT = exports.testPreviewImages = exports.previewWordAsPdf = exports.previewExcelAsHtml = exports.previewDocxAsHtml = exports.openFileLocally = exports.createBlankDocument = exports.updateFileMetadata = exports.deleteFile = exports.downloadFile = exports.uploadFiles = exports.listFiles = void 0;
...
function ensureLocalEditableDir(clientId, attachmentType, fileId) {
    const baseDir = ensureLocalClientDir(clientId, attachmentType);
    if (!fileId)
        return baseDir;
    const dir = path_1.default.join(baseDir, fileId);
    if (!fs_1.default.existsSync(dir))
        fs_1.default.mkdirSync(dir, { recursive: true });
    return dir;
}
function closeLocalEditWatcher(localPath) {
...
function watchLocalEditableFile(localPath, serverPath, fileId, clientId) {
    closeLocalEditWatcher(localPath);
    let lastSyncedMtimeMs = fs_1.default.existsSync(localPath) ? fs_1.default.statSync(localPath).mtimeMs : 0;
    const syncBackToServer = async () => {
        try {
            if (!fs_1.default.existsSync(localPath) || !fs_1.default.existsSync(serverPath))
                return;
            const localStat = fs_1.default.statSync(localPath);
            if (localStat.mtimeMs <= lastSyncedMtimeMs)
                return;
            fs_1.default.copyFileSync(localPath, serverPath);
            lastSyncedMtimeMs = fs_1.default.statSync(localPath).mtimeMs;
            await database_1.default.query(`UPDATE client_files
         SET size_bytes = $1
         WHERE id = $2 AND client_id = $3`, [localStat.size, fileId, clientId]);
        }
        catch (_) {
        }
    };
    const watcher = fs_1.default.watch(localPath, () => {
...
const uploadFiles = async (req, res) => {
    ...
    try {
        ...
        for (const file of files) {
            const baseFileName = path_1.default.basename(file.originalname);
            const result = await database_1.default.query(`INSERT INTO client_files (client_id, original_name, stored_name, mimetype, size_bytes, created_by)
...
const createBlankDocument = async (req, res) => {
    const { clientId } = req.params;
    const { document_name, attachment_type } = req.body;
    const userId = req.auth?.userId || 'SYSTEM';
    try {
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id, original_name`, [clientId, originalName, storedName, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', blankDocx.length, document_name || null, attachment_type || 'Sin clasificar', userId]);
        ...
        (0, activityController_1.logActivityForReq)(req, `Documento creado: ${originalName}`, 'CLIENT', clientId);
        res.status(201).json({
            success: true,
            data: {
                id: fileId,
...
exports.createBlankDocument = createBlankDocument;
const openFileLocally = async (req, res) => {
    const { clientId, fileId } = req.params;
    try {
        const result = await database_1.default.query(`SELECT stored_name, original_name, attachment_type FROM client_files WHERE id = $1 AND client_id = $2`, [fileId, clientId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Archivo no encontrado.' });
        }
        const { stored_name, original_name, attachment_type } = result.rows[0];
        ...
        const serverStat = fs_1.default.statSync(serverPath);
        const shouldRefreshLocal = !fs_1.default.existsSync(localPath) ||
            fs_1.default.statSync(localPath).mtimeMs < serverStat.mtimeMs;
...
exports.openFileLocally = openFileLocally;
const previewDocxAsHtml = async (req, res) => {
    const { clientId, fileId } = req.params;
    try {
        const result = await database_1.default.query(`SELECT stored_name, original_name, mimetype FROM client_files WHERE id = $1 AND client_id = $2`, [fileId, clientId]);
        if (result.rows.length === 0) {
...
`;
        ...
        execFile(pythonCmd, [scriptPath, filePath], { timeout: 10000, maxBuffer: 10 * 1024 * 1024 }, (error, stdout, stderr) => {
            try {
                try {
...
exports.previewDocxAsHtml = previewDocxAsHtml;
const previewExcelAsHtml = async (req, res) => {
    const { clientId, fileId } = req.params;
    try {
        const result = await database_1.default.query(`SELECT stored_name, original_name, mimetype FROM client_files WHERE id = $1 AND client_id = $2`, [fileId, clientId]);
        if (result.rows.length === 0) {
        ...
        const tempDir = path_1.default.join(require('os').tmpdir(), 'lextech_previews');
        if (!fs_1.default.existsSync(tempDir))
            fs_1.default.mkdirSync(tempDir, { recursive: true });
        const scriptPath = path_1.default.join(tempDir, `excel_preview_${fileId}.py`);
...
`;
        fs_1.default.writeFileSync(scriptPath, pythonScript, { encoding: 'utf8' });
        const { execFile } = require('child_process');
...
<body style="background:#fffbf0;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;font-family:Calibri,Arial,sans-serif;">
  <div style="text-align:center;padding:32px 40px;background:white;border:1px solid #fcd34d;border-radius:12px;max-width:380px;box-shadow:0 2px 12px rgba(0,0,0,.06)">
    ...
    <p style="font-size:12px;color:#a16207;margin:0;line-height:1.6">Instala Python 3 para poder previsualizar archivos Excel.</p>
            ...
            execFile(cmd, [scriptPath, filePath], { timeout: 60000, maxBuffer: 50 * 1024 * 1024 }, (err, stdout, stderr) => {
...
<body style="padding:20px;font-family:Calibri,Arial,sans-serif;color:#b91c1c">
  ...
  <pre style="background:#fff3f3;border:1px solid #fca5a5;border-radius:6px;padding:12px;font-size:12px;overflow-x:auto;white-space:pre-wrap">${detail}</pre>
        ...
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        res.status(200).send(`<!DOCTYPE html><html><head><meta charset="utf-8"></head>
<body style="padding:20px;font-family:Calibri,Arial,sans-serif;color:#b91c1c">
...
const previewWordAsPdf = async (req, res) => {
    ...
    try {
        ...
        const srcStat = fs_1.default.statSync(sourcePath);
        const cacheKey = Buffer.from(`${clientId}:${fileId}:${sourcePath}`).toString('hex').slice(0, 32);
        const tempDir = path_1.default.join(paths_1.TEMP_ROOT, `word_preview_${cacheKey}`);
        if (!fs_1.default.existsSync(tempDir))
            fs_1.default.mkdirSync(tempDir, { recursive: true });
        const outputPdf = path_1.default.join(tempDir, `preview.pdf`);
        if (fs_1.default.existsSync(outputPdf)) {
            const pdfStat = fs_1.default.statSync(outputPdf);
        ...
        try {
            if (fs_1.default.existsSync(outputPdf))
                fs_1.default.unlinkSync(outputPdf);
        }
        catch (_) { }
        const converterScript = path_1.default.join(tempDir, `word_to_pdf_${cacheKey}.py`);
...
`;
        ...
        const tryPython = (index) => {
            ...
            execFile(pythonCmds[index], [converterScript, sourcePath, outputPdf], { timeout: 120000 }, (err) => {
                const success = !err && fs_1.default.existsSync(outputPdf);
                if (!success)
...
exports.previewWordAsPdf = previewWordAsPdf;
const testPreviewImages = async (req, res) => {
...
function scanDocPlant(dir, relBase) {
    const out = [];
    if (!fs_1.default.existsSync(dir))
        return out;
    try {
        ...
        for (const e of entries) {
            ...
            if (e.isDirectory()) {
                ...
                out.push({ name: e.name, path: relPath, ext: path_1.default.extname(e.name).toLowerCase() });
            }
        }
    }
    catch (_e) { }
...
const previewTemplateAsHtml = async (req, res) => {
    const relPath = req.query.path;
    if (!relPath)
        return res.status(400).json({ success: false, error: 'Parámetro path requerido.' });
    const resolved = path_1.default.resolve(DOCPLANT_ROOT, relPath);
    const docplantRoot = path_1.default.resolve(DOCPLANT_ROOT);
    if (!resolved.startsWith(docplantRoot + path_1.default.sep) && resolved !== docplantRoot) {
    ...
    const runDocxPreview = (docxPath) => {
        try {
            ...
            const scriptPath = path_1.default.join(tempDir, `tpl_${scriptId}.py`);
            const pythonScript = `#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import sys, io, base64
from zipfile import ZipFile
...
try:
    ...
    print('<html><body style="background: #fff3cd;"><div style="padding: 20px; color: #856404;"><h3>Error al procesar</h3><pre style="background: #fff; padding: 10px; border-radius: 4px; overflow-x: auto; font-size: 12px;">' + error_msg.replace('<', '&lt;').replace('>', '&gt;') + '</pre></div></body></html>')
`;
            fs_1.default.writeFileSync(scriptPath, pythonScript, { encoding: 'utf-8' });
...
<body style="background:#fff3cd;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;font-family:Calibri,Arial,sans-serif;">
  <div style="text-align:center;padding:28px 36px;background:white;border:1px solid #fcd34d;border-radius:12px;max-width:380px">
    <p style="font-size:13px;color:#92400e;margin:0">No se pudo generar la vista previa.<br>Asegúrate de que Python esté instalado.</p>
                ...
                execFile(cmds[i], [scriptPath, docxPath], { timeout: 20000, maxBuffer: 10 * 1024 * 1024 }, (error, stdout) => {
                    if (error) {
                        tryCmd(i + 1);
                        return;
                    }
    ...
    if (ext === '.doc') {
        ...
        const outputDocx = path_1.default.join(tempDir, `doc_converted_${scriptId2}.docx`);
        try {
            if (fs_1.default.existsSync(outputDocx)) {
                const srcMtime = fs_1.default.statSync(resolved).mtimeMs;
                const cacheMtime = fs_1.default.statSync(outputDocx).mtimeMs;
        ...
        const converterScript = path_1.default.join(tempDir, `converter_${scriptId2}.py`);
        const converterCode = `import sys, os, subprocess, shutil
  ...
  <div style="text-align:center;padding:32px 40px;background:white;border:1px solid #fcd34d;border-radius:12px;max-width:380px;box-shadow:0 2px 12px rgba(0,0,0,.06)">
    ...
    <p style="font-size:12px;color:#a16207;margin:0;line-height:1.6">
      ...
      Descárgalo para abrirlo directamente en Word.
            ...
            execFile(pythonCmds[i], [converterScript, resolved, outputDocx], { timeout: 90000 }, (err, stdout) => {
                const success = !err && fs_1.default.existsSync(outputDocx);
                if (!success)
                    return tryPythonConverter(i + 1);
                try {
                    fs_1.default.unlinkSync(converterScript);
                }
                catch (_) { }
...
const previewTemplateAsPdf = async (req, res) => {
    ...
    try {
        const srcStat = fs_1.default.statSync(resolved);
        const cacheKey = Buffer.from(`tpl:${relPath}:${resolved}`).toString('hex').slice(0, 32);
        const tempDir = path_1.default.join(paths_1.TEMP_ROOT, `tpl_preview_${cacheKey}`);
        if (!fs_1.default.existsSync(tempDir))
            fs_1.default.mkdirSync(tempDir, { recursive: true });
        const outputPdf = path_1.default.join(tempDir, `preview.pdf`);
        if (fs_1.default.existsSync(outputPdf)) {
            ...
            if (pdfStat.mtimeMs >= srcStat.mtimeMs) {
                res.setHeader('Content-Type', 'application/pdf');
                res.setHeader('Content-Disposition', `inline; filename="${encodeURIComponent(path_1.default.basename(resolved).replace(/\.(docx?|DOCX?)$/, '.pdf'))}"`);
                return res.sendFile(outputPdf);
            }
        }
        try {
        ...
        const converterScript = path_1.default.join(tempDir, `tpl_word_to_pdf_${cacheKey}.py`);
        const converterCode = `import sys, os, subprocess, shutil
...
`;
        ...
        const tryPython = (index) => {
            ...
            execFile(pythonCmds[index], [converterScript, resolved, outputPdf], { timeout: 120000 }, (err) => {
                const success = !err && fs_1.default.existsSync(outputPdf);
                if (!success)
                    return tryPython(index + 1);
                try {
...
exports.previewTemplateAsPdf = previewTemplateAsPdf;
const downloadTemplate = (req, res) => {
    const filePath = req.query.path;
    if (!filePath)
        return res.status(400).json({ success: false, error: 'Parámetro path requerido.' });
    const resolved = path_1.default.resolve(DOCPLANT_ROOT, filePath);
    const root = path_1.default.resolve(DOCPLANT_ROOT);
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(path_1.default.basename(resolved))}"`);
    res.sendFile(resolved);
};
exports.downloadTemplate = downloadTemplate;
const downloadBlank = (_req, res) => {
...
exports.downloadBlank = downloadBlank;
