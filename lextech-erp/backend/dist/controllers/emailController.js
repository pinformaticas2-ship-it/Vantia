...
exports.getOAuthProfiles = getOAuthProfiles;
exports.upsertOAuthProfile = upsertOAuthProfile;
exports.deleteOAuthProfile = deleteOAuthProfile;
exports.createAccount = createAccount;
exports.deleteAccount = deleteAccount;
exports.getAccountFolders = getAccountFolders;
exports.createAccountFolder = createAccountFolder;
...
exports.getRecipientSuggestions = getRecipientSuggestions;
exports.logGmailSent = logGmailSent;
...
const pop3_1 = require("../utils/pop3");
...
function explainMailConnectionError(error, proto) {
    const message = [
        error?.responseText,
        error?.response,
...
async function upsertEmailContacts(uid, contacts, source = 'gmail') {
    for (const contact of contacts) {
        await database_1.default.query(`INSERT INTO email_contacts (user_id, email, name, source, usage_count, last_used_at)
       ...
       DO UPDATE SET
         ...
         last_used_at = NOW(),
         updated_at = NOW()`, [uid, contact.email, contact.name || null, source]);
    }
}
async function getAccountForMessage(messageId, uid) {
    const { rows } = await database_1.default.query(`SELECT e.id, e.uid, e.folder, e.user_id,
      ...
      WHERE e.id=$1 AND e.user_id=$2`, [messageId, uid]);
    return rows[0] || null;
}
async function getAccounts(req, res) {
    ...
    try {
        const { rows } = await database_1.default.query(`SELECT id, label, email, imap_host, imap_port, imap_secure,
              smtp_host, smtp_port, smtp_secure, username, active,
              COALESCE(protocol, 'imap') AS protocol,
              last_sync_at, created_at
       FROM email_accounts WHERE user_id=$1 ORDER BY created_at`, [uid]);
        return ok(res, rows);
    }
    catch (e) {
...
async function getOAuthProfiles(req, res) {
    const uid = userId(req);
    if (!uid)
        return err(res, 'No autenticado', 401);
    const provider = String(req.query.provider || 'google').toLowerCase();
    try {
        const { rows } = await database_1.default.query(`SELECT id, provider, email, display_name, avatar_url, external_id, last_used_at, created_at
         FROM email_oauth_profiles
        WHERE user_id=$1 AND provider=$2
        ORDER BY last_used_at DESC, created_at DESC`, [uid, provider]);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
...
async function upsertOAuthProfile(req, res) {
    const uid = userId(req);
    if (!uid)
        return err(res, 'No autenticado', 401);
    ...
    if (!email)
        return err(res, 'Falta el email del perfil', 400);
    try {
        const { rows } = await database_1.default.query(`INSERT INTO email_oauth_profiles (user_id, provider, email, display_name, avatar_url, external_id, last_used_at)
       ...
       DO UPDATE SET
         ...
         last_used_at = NOW(),
         updated_at   = NOW()
       RETURNING id, provider, email, display_name, avatar_url, external_id, last_used_at, created_at`, [uid, provider, email, displayName, avatarUrl, externalId]);
...
async function deleteOAuthProfile(req, res) {
    const uid = userId(req);
    if (!uid)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    try {
        const { rowCount } = await database_1.default.query(`DELETE FROM email_oauth_profiles WHERE id=$1 AND user_id=$2`, [id, uid]);
        if (!rowCount)
...
async function createAccount(req, res) {
    ...
    if (!uid)
        return err(res, 'No autenticado', 401);
    const { label = 'Mi cuenta', email, imap_host, imap_port, imap_secure = true, smtp_host, smtp_port = 587, smtp_secure = false, username, password, protocol = 'imap', } = req.body;
    ...
    try {
        const { rows: existingAccounts } = await database_1.default.query(`SELECT id, label
         FROM email_accounts
        WHERE user_id=$1
          ...
          AND active=true
        LIMIT 1`, [uid, email, username, imap_host, proto]);
        ...
        const withTimeout = (p, ms, label) => Promise.race([p, new Promise((_, rej) => setTimeout(() => rej(new Error(`Timeout (${label}): el servidor tardó demasiado en responder`)), ms))]);
        if (proto === 'pop3') {
            const pop3Cfg = {
            ...
            await withTimeout((0, pop3_1.testPop3Connection)(pop3Cfg), 20000, 'POP3').catch((e) => {
            ...
            await withTimeout((0, imap_1.testImapConnection)(imapCfg), 20000, 'IMAP').catch((e) => {
        ...
        await withTimeout((0, smtp_1.testSmtpConnection)(smtpCfg), 20000, 'SMTP').catch((e) => {
            throw new Error(explainMailConnectionError(e, 'smtp'));
        });
        const enc = encryptPassword(password);
        const { rows } = await database_1.default.query(`INSERT INTO email_accounts
       ...
       RETURNING id, label, email, imap_host, imap_port, imap_secure,
                 smtp_host, smtp_port, smtp_secure, username, active,
                 protocol, last_sync_at, created_at`, [uid, label, email, imap_host, inPort, Boolean(imap_secure),
...
async function deleteAccount(req, res) {
    ...
    const { id } = req.params;
    try {
        const { rowCount } = await database_1.default.query(`DELETE FROM email_accounts WHERE id=$1 AND user_id=$2`, [id, uid]);
        if (!rowCount)
            return err(res, 'Cuenta no encontrada', 404);
        return ok(res, { id });
    }
    catch (e) {
        return err(res, e.message);
...
async function getAccountFolders(req, res) {
    const uid = userId(req);
    if (!uid)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    try {
        const { rows } = await database_1.default.query(`SELECT * FROM email_accounts WHERE id=$1 AND user_id=$2 AND active=true`, [id, uid]);
        if (!rows.length)
            return err(res, 'Cuenta no encontrada', 404);
        const acc = rows[0];
        ...
        const client = new imap_1.ImapClient(cfg);
        try {
...
async function createAccountFolder(req, res) {
    const uid = userId(req);
    if (!uid)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    ...
    if (!name)
        return err(res, 'Indica un nombre de carpeta', 400);
    try {
        const { rows } = await database_1.default.query(`SELECT * FROM email_accounts WHERE id=$1 AND user_id=$2 AND active=true`, [id, uid]);
        if (!rows.length)
            return err(res, 'Cuenta no encontrada', 404);
        ...
        const proto = String(acc.protocol || 'imap').toLowerCase();
        if (proto === 'pop3')
            return err(res, 'Las cuentas POP3 no permiten crear carpetas', 400);
        const password = decryptPassword(acc.password_enc);
        const cfg = {
            host: acc.imap_host,
            port: acc.imap_port,
...
async function syncAccount(req, res) {
    ...
    try {
        const { rows: accs } = await database_1.default.query(`SELECT * FROM email_accounts WHERE id=$1 AND user_id=$2 AND active=true`, [id, uid]);
        if (!accs.length)
            return err(res, 'Cuenta no encontrada', 404);
        const acc = accs[0];
        const password = decryptPassword(acc.password_enc);
        ...
        let synced = 0;
        if (proto === 'pop3') {
            const { rows: known } = await database_1.default.query(`SELECT message_id FROM emails WHERE account_id=$1 AND message_id IS NOT NULL`, [acc.id]);
            const knownUidls = new Set(known.map((r) => String(r.message_id)));
            const pop3Cfg = {
                host: acc.imap_host, port: acc.imap_port, secure: acc.imap_secure,
                user: acc.username, password,
            };
            const messages = await (0, pop3_1.syncPop3Inbox)(pop3Cfg, knownUidls, limit);
            synced = messages.length;
            for (const msg of messages) {
                const sentAt = msg.date ? new Date(msg.date) : null;
                const { rowCount } = await database_1.default.query(`INSERT INTO emails
           ...
           ON CONFLICT DO NOTHING`, [
            ...
            const messages = await (0, imap_1.syncInbox)(imapCfg, folder, limit);
            synced = messages.length;
            for (const msg of messages) {
                const sentAt = msg.date ? new Date(msg.date) : null;
                const { rowCount } = await database_1.default.query(`INSERT INTO emails
        ...
        await database_1.default.query(`UPDATE email_accounts SET last_sync_at=NOW() WHERE id=$1`, [acc.id]);
        return ok(res, { synced, inserted, folder, protocol: proto });
    }
    catch (e) {
        return err(res, `Error de sincronización: ${e.message}`);
    }
}
async function getMessages(req, res) {
    ...
    try {
        ...
        if (folder === 'STARRED') {
            ...
            params.push(folder);
            conditions.push(`e.folder=$${params.length}`);
        }
        if (onlyUnread)
            conditions.push(`e.is_read=false`);
        if (onlyStarred)
        ...
        const { rows } = await database_1.default.query(`SELECT e.id, e.uid, e.folder, e.from_email, e.from_name, e.to_emails,
       ...
       LIMIT $${limitP} OFFSET $${offsetP}`, params);
        const { rows: cnt } = await database_1.default.query(`SELECT COUNT(*) FROM emails e WHERE ${where}`, params.slice(0, -2));
...
async function getMessage(req, res) {
    ...
    const { id } = req.params;
    try {
        const { rows } = await database_1.default.query(`SELECT e.*, a.imap_host, a.imap_port, a.imap_secure,
              a.username, a.password_enc, a.email AS account_email, a.label AS account_label
       FROM emails e
       JOIN email_accounts a ON a.id=e.account_id
       WHERE e.id=$1 AND e.user_id=$2`, [id, uid]);
        if (!rows.length)
            return err(res, 'Email no encontrado', 404);
        const row = rows[0];
        if (row.uid && !row.body_html && !row.body_text) {
            try {
                ...
                await client.logout();
                if (full) {
                    await database_1.default.query(`UPDATE emails SET body_text=$1, body_html=$2, snippet=$3 WHERE id=$4`, [full.bodyText, full.bodyHtml, full.snippet, id]);
        ...
        if (!row.is_read) {
            await database_1.default.query(`UPDATE emails SET is_read=true WHERE id=$1`, [id]);
            row.is_read = true;
            if (row.uid) {
                try {
                    const password = decryptPassword(row.password_enc);
...
async function markRead(req, res) {
    ...
    try {
        ...
        if (!message)
            return err(res, 'Email no encontrado', 404);
        await database_1.default.query(`UPDATE emails SET is_read=$1 WHERE id=$2 AND user_id=$3`, [Boolean(read), id, uid]);
        if (message.uid) {
            try {
...
async function toggleStar(req, res) {
    ...
    try {
        ...
        if (!message)
            return err(res, 'Email no encontrado', 404);
        const { rows } = await database_1.default.query(`UPDATE emails SET is_starred = NOT is_starred WHERE id=$1 AND user_id=$2
       RETURNING is_starred`, [id, uid]);
        if (!rows.length)
            return err(res, 'Email no encontrado', 404);
        if (message.uid) {
            try {
...
async function deleteMessage(req, res) {
    ...
    try {
        const { rows } = await database_1.default.query(`SELECT e.uid, e.folder, a.imap_host, a.imap_port, a.imap_secure, a.username, a.password_enc
       FROM emails e JOIN email_accounts a ON a.id=e.account_id
       WHERE e.id=$1 AND e.user_id=$2`, [id, uid]);
        if (!rows.length)
            return err(res, 'Email no encontrado', 404);
        if (permanent) {
            await database_1.default.query(`DELETE FROM emails WHERE id=$1 AND user_id=$2`, [id, uid]);
        }
        else {
            await database_1.default.query(`UPDATE emails SET folder='Trash' WHERE id=$1 AND user_id=$2`, [id, uid]);
        }
        const row = rows[0];
        if (row.uid) {
            try {
...
async function sendMail(req, res) {
    ...
    try {
        if (!accountId) {
            const { rows: firstAccountRows } = await database_1.default.query(`SELECT id FROM email_accounts WHERE user_id=$1 AND active=true ORDER BY created_at ASC LIMIT 1`, [uid]);
            accountId = firstAccountRows[0]?.id;
        }
        if (!accountId)
            return err(res, 'No hay ninguna cuenta IMAP/SMTP activa para enviar', 400);
        const { rows } = await database_1.default.query(`SELECT * FROM email_accounts WHERE id=$1 AND user_id=$2 AND active=true`, [accountId, uid]);
        if (!rows.length)
            return err(res, 'Cuenta no encontrada', 404);
        const acc = rows[0];
        const password = decryptPassword(acc.password_enc);
        ...
        await (0, smtp_1.sendEmail)(smtpCfg, mailMsg);
        await database_1.default.query(`INSERT INTO emails (account_id, user_id, folder, from_email, from_name,
       ...
       VALUES ($1,$2,'Sent',$3,$4,$5,$6,$7,$8,$9,$10,true,NOW())`, [
        ...
        if (draft_id) {
            await database_1.default.query(`DELETE FROM emails WHERE id=$1 AND user_id=$2 AND is_draft=true`, [draft_id, uid]);
        }
        ...
        await (0, activityController_1.logActivityForReq)(req, `Correo enviado: ${subject}`, 'EMAIL', accountId, (Array.isArray(toList) ? toList : [toList]).join(', '), 'CREATE');
        return ok(res, { sent: true });
    }
    catch (e) {
        return err(res, `Error al enviar: ${e.message}`);
    }
}
async function getRecipientSuggestions(req, res) {
    const uid = userId(req);
    if (!uid)
        return err(res, 'No autenticado', 401);
...
async function logGmailSent(req, res) {
    const uid = userId(req);
    if (!uid)
        return err(res, 'No autenticado', 401);
    const { to, cc, bcc, subject, snippet, has_attachments } = req.body || {};
    if (!to || !subject)
        return err(res, 'Faltan campos obligatorios (to, subject)', 400);
    try {
        const contacts = [
            ...extractContacts(to),
            ...extractContacts(cc),
        ...
        await (0, activityController_1.logActivityForReq)(req, `Correo Gmail enviado${attachmentSuffix}: ${subject}`, 'EMAIL', undefined, recipientsText || subject, 'CREATE');
        return ok(res, {
            logged: true,
...
async function saveDraft(req, res) {
    ...
    try {
        if (id) {
            const { rows } = await database_1.default.query(`UPDATE emails SET to_emails=$1, cc_emails=$2, subject=$3, body_html=$4, sent_at=NOW()
         WHERE id=$5 AND user_id=$6 AND is_draft=true RETURNING id`, [to || '', cc || '', subject || '(Sin asunto)', html || '', id, uid]);
            return ok(res, rows[0] || { id });
        }
        const { rows } = await database_1.default.query(`INSERT INTO emails (account_id, user_id, folder, to_emails, cc_emails,
        subject, body_html, is_draft, is_read, sent_at)
       VALUES ($1,$2,'Drafts',$3,$4,$5,$6,true,true,NOW())
       RETURNING id`, [account_id || null, uid, to || '', cc || '', subject || '(Sin asunto)', html || '']);
        return ok(res, rows[0]);
    }
    catch (e) {
        return err(res, e.message);
...
async function getDrafts(req, res) {
    ...
    if (!uid)
        return err(res, 'No autenticado', 401);
    try {
        const { rows } = await database_1.default.query(`SELECT id, to_emails, cc_emails, subject, body_html, sent_at, account_id
       FROM emails WHERE user_id=$1 AND is_draft=true ORDER BY sent_at DESC`, [uid]);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
...
async function getStats(req, res) {
    ...
    try {
       ...
       FROM emails WHERE user_id=$1 ${accCond}`, params);
        return ok(res, rows[0]);
    }
    catch (e) {
        return err(res, e.message);
