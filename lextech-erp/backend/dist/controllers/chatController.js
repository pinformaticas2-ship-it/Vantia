"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getCanales = getCanales;
exports.createCanal = createCanal;
exports.updateCanal = updateCanal;
exports.archivarCanal = archivarCanal;
exports.marcarLeido = marcarLeido;
exports.marcarTodoLeido = marcarTodoLeido;
exports.getCanalMiembros = getCanalMiembros;
exports.updateMyStatus = updateMyStatus;
exports.updateMyRole = updateMyRole;
exports.getMensajes = getMensajes;
exports.sendMensaje = sendMensaje;
exports.uploadChatImage = uploadChatImage;
exports.getTypingStatus = getTypingStatus;
exports.updateTypingStatus = updateTypingStatus;
exports.editMensaje = editMensaje;
exports.deleteMensaje = deleteMensaje;
exports.toggleReaccion = toggleReaccion;
exports.getFijados = getFijados;
exports.fijarMensaje = fijarMensaje;
exports.desfijarMensaje = desfijarMensaje;
exports.getFavoritos = getFavoritos;
exports.toggleFavorito = toggleFavorito;
exports.buscarMensajes = buscarMensajes;
exports.getMiembrosGlobal = getMiembrosGlobal;
exports.getOrCreateDM = getOrCreateDM;
exports.getCanalesDisponibles = getCanalesDisponibles;
exports.joinCanal = joinCanal;
exports.leaveCanal = leaveCanal;
exports.addMiembro = addMiembro;
exports.removeMiembro = removeMiembro;
exports.updateMiembroRole = updateMiembroRole;
exports.getSystemUsers = getSystemUsers;
exports.buscarCanalesDisponibles = buscarCanalesDisponibles;
exports.getUnreadCounts = getUnreadCounts;
const path_1 = __importDefault(require("path"));
const database_1 = __importDefault(require("../config/database"));
const ok = (res, data, status = 200) => res.status(status).json({ success: true, data });
const err = (res, msg, status = 500) => res.status(status).json({ success: false, error: msg });
const resolveAuthDisplayName = (auth) => {
    const claims = auth?.sessionClaims || {};
    const nameParts = [claims.first_name, claims.last_name].filter(Boolean);
    const resolved = [
        claims.name,
        nameParts.length ? nameParts.join(' ') : null,
        claims.username,
        claims.email,
    ].find(value => typeof value === 'string' && value.trim());
    return resolved || 'Sin nombre';
};
const unreadCountExpr = (userParam = '$1') => `
  (SELECT COUNT(*) FROM chat_mensajes msg
    WHERE msg.canal_id = c.id
      AND msg.deleted_at IS NULL
      AND msg.user_id != ${userParam}
      AND (m.last_read_at IS NULL OR msg.created_at > m.last_read_at)
  )::int
`;
async function getCanales(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    try {
        const { rows } = await database_1.default.query(`
      SELECT
        c.id, c.nombre, c.descripcion, c.tipo,
        c.expediente_id, c.cliente_id, c.archivado, c.created_at,
        m.last_read_at, m.role_label, m.status AS my_status,
        ${unreadCountExpr('$1')} AS no_leidos,
        (SELECT contenido FROM chat_mensajes msg
          WHERE msg.canal_id = c.id AND msg.deleted_at IS NULL
          ORDER BY msg.created_at DESC LIMIT 1
        ) AS ultimo_mensaje,
        (SELECT user_name FROM chat_mensajes msg
          WHERE msg.canal_id = c.id AND msg.deleted_at IS NULL
          ORDER BY msg.created_at DESC LIMIT 1
        ) AS ultimo_mensaje_autor,
        (SELECT created_at FROM chat_mensajes msg
          WHERE msg.canal_id = c.id AND msg.deleted_at IS NULL
          ORDER BY msg.created_at DESC LIMIT 1
        ) AS ultimo_mensaje_at,
        (SELECT COUNT(*) FROM chat_miembros WHERE canal_id = c.id)::int AS total_miembros,
        (SELECT dm.user_id
          FROM chat_miembros dm
          WHERE dm.canal_id = c.id AND dm.user_id != $1
          ORDER BY dm.joined_at ASC
          LIMIT 1
        ) AS dm_target_user_id,
        (SELECT dm.user_name
          FROM chat_miembros dm
          WHERE dm.canal_id = c.id AND dm.user_id != $1
          ORDER BY dm.joined_at ASC
          LIMIT 1
        ) AS dm_target_user_name,
        (SELECT dm.avatar_url
          FROM chat_miembros dm
          WHERE dm.canal_id = c.id AND dm.user_id != $1
          ORDER BY dm.joined_at ASC
          LIMIT 1
        ) AS dm_target_avatar_url
      FROM chat_canales c
      JOIN chat_miembros m ON m.canal_id = c.id AND m.user_id = $1
      WHERE c.archivado = false
      ORDER BY COALESCE(c.updated_at, c.created_at) DESC, c.nombre ASC
    `, [userId]);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function createCanal(req, res) {
    const userId = req.auth?.userId;
    const userName = resolveAuthDisplayName(req.auth);
    const avatarUrl = req.auth?.sessionClaims?.picture || null;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { nombre, descripcion, tipo = 'publico', expediente_id, cliente_id } = req.body;
    if (!nombre?.trim())
        return err(res, 'Nombre requerido', 400);
    try {
        const { rows } = await database_1.default.query(`
      INSERT INTO chat_canales (nombre, descripcion, tipo, expediente_id, cliente_id, created_by)
      VALUES ($1, $2, $3, $4, $5, $6) RETURNING *
    `, [nombre.trim(), descripcion || null, tipo, expediente_id || null, cliente_id || null, userId]);
        const canal = rows[0];
        await database_1.default.query(`
      INSERT INTO chat_miembros (canal_id, user_id, user_name, avatar_url, role, role_label, last_read_at)
      VALUES ($1, $2, $3, $4, 'admin', 'Administrador', NOW())
      ON CONFLICT (canal_id, user_id) DO NOTHING
    `, [canal.id, userId, userName, avatarUrl]);
        return ok(res, canal, 201);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function updateCanal(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    const { nombre, descripcion } = req.body;
    try {
        const { rows } = await database_1.default.query(`
      UPDATE chat_canales SET nombre = COALESCE($1, nombre), descripcion = COALESCE($2, descripcion), updated_at = NOW()
      WHERE id = $3 RETURNING *
    `, [nombre?.trim() || null, descripcion ?? null, id]);
        if (!rows.length)
            return err(res, 'Canal no encontrado', 404);
        return ok(res, rows[0]);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function archivarCanal(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    try {
        await database_1.default.query(`UPDATE chat_canales SET archivado = true WHERE id = $1`, [id]);
        return ok(res, { id });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function marcarLeido(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    try {
        await database_1.default.query(`UPDATE chat_miembros SET last_read_at = NOW() WHERE canal_id = $1 AND user_id = $2`, [id, userId]);
        return ok(res, { ok: true });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function marcarTodoLeido(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    try {
        await database_1.default.query(`UPDATE chat_miembros SET last_read_at = NOW() WHERE user_id = $1`, [userId]);
        return ok(res, { ok: true });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function getCanalMiembros(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    try {
        const { rows } = await database_1.default.query(`
      SELECT user_id, user_name, avatar_url, role, role_label, status, joined_at, last_read_at
      FROM chat_miembros WHERE canal_id = $1
      ORDER BY role DESC, user_name ASC
    `, [id]);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function updateMyStatus(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { status } = req.body;
    const valid = ['disponible', 'ocupado', 'no_molestar', 'en_juicio', 'en_reunion', 'ausente'];
    if (!valid.includes(status))
        return err(res, 'Status no válido', 400);
    try {
        await database_1.default.query(`UPDATE chat_miembros SET status = $1 WHERE user_id = $2`, [status, userId]);
        return ok(res, { status });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function updateMyRole(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { role_label, canal_id } = req.body;
    if (!role_label?.trim())
        return err(res, 'Role requerido', 400);
    try {
        if (canal_id) {
            await database_1.default.query(`UPDATE chat_miembros SET role_label = $1 WHERE user_id = $2 AND canal_id = $3`, [role_label, userId, canal_id]);
        }
        else {
            await database_1.default.query(`UPDATE chat_miembros SET role_label = $1 WHERE user_id = $2`, [role_label, userId]);
        }
        return ok(res, { role_label });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function getMensajes(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    const { before, since, limit = '60' } = req.query;
    const lim = Math.min(parseInt(limit) || 60, 100);
    try {
        let query;
        let params;
        const SEL = `
      SELECT m.*,
        (SELECT json_agg(json_build_object('emoji', r.emoji, 'user_id', r.user_id, 'user_name', r.user_name))
         FROM chat_reacciones r WHERE r.mensaje_id = m.id
        ) AS reacciones,
        (SELECT row_to_json(r2.*) FROM chat_mensajes r2 WHERE r2.id = m.reply_to_id) AS reply_to
      FROM chat_mensajes m
    `;
        if (since) {
            query = `${SEL} WHERE m.canal_id = $1 AND m.created_at > $2 AND m.deleted_at IS NULL ORDER BY m.created_at ASC LIMIT $3`;
            params = [id, since, lim];
        }
        else if (before) {
            query = `${SEL} WHERE m.canal_id = $1 AND m.created_at < (SELECT created_at FROM chat_mensajes WHERE id = $2) AND m.deleted_at IS NULL ORDER BY m.created_at DESC LIMIT $3`;
            params = [id, before, lim];
        }
        else {
            query = `${SEL} WHERE m.canal_id = $1 AND m.deleted_at IS NULL ORDER BY m.created_at DESC LIMIT $2`;
            params = [id, lim];
        }
        const { rows } = await database_1.default.query(query, params);
        const msgs = before ? rows.reverse() : since ? rows : rows.reverse();
        return ok(res, msgs);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function sendMensaje(req, res) {
    const userId = req.auth?.userId;
    const userName = resolveAuthDisplayName(req.auth);
    const avatarUrl = req.auth?.sessionClaims?.picture || null;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    const { contenido, tipo = 'texto', reply_to_id, gif_url, image_url } = req.body;
    if (!contenido?.trim() && !gif_url && !image_url)
        return err(res, 'Contenido vacío', 400);
    try {
        const normalizedType = image_url ? 'imagen' : gif_url ? 'gif' : tipo;
        const fallbackContent = gif_url ? 'GIF' : image_url ? 'Imagen' : '';
        const { rows } = await database_1.default.query(`
      INSERT INTO chat_mensajes (canal_id, user_id, user_name, avatar_url, contenido, tipo, reply_to_id, gif_url, image_url)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *
    `, [id, userId, userName, avatarUrl, (contenido || '').trim() || fallbackContent, normalizedType, reply_to_id || null, gif_url || null, image_url || null]);
        const msg = { ...rows[0], reacciones: null, reply_to: null };
        if (reply_to_id) {
            const { rows: r } = await database_1.default.query(`SELECT * FROM chat_mensajes WHERE id = $1`, [reply_to_id]);
            msg.reply_to = r[0] || null;
        }
        await database_1.default.query(`UPDATE chat_miembros SET last_read_at = NOW() WHERE canal_id = $1 AND user_id = $2`, [id, userId]);
        await database_1.default.query(`DELETE FROM chat_typing_status WHERE canal_id = $1 AND user_id = $2`, [id, userId]);
        await database_1.default.query(`UPDATE chat_miembros SET avatar_url = COALESCE($1, avatar_url), user_name = $2 WHERE canal_id = $3 AND user_id = $4`, [avatarUrl, userName, id, userId]);
        await database_1.default.query(`UPDATE chat_canales SET updated_at = NOW() WHERE id = $1`, [id]);
        return ok(res, msg, 201);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function uploadChatImage(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const file = req.file;
    if (!file)
        return err(res, 'Imagen requerida', 400);
    const imageUrl = path_1.default.posix.join('/uploads', 'chat', file.filename);
    return ok(res, {
        image_url: imageUrl,
        original_name: file.originalname,
        mimetype: file.mimetype,
        size: file.size,
    }, 201);
}
async function getTypingStatus(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    try {
        await database_1.default.query(`DELETE FROM chat_typing_status WHERE canal_id = $1 AND updated_at < NOW() - INTERVAL '10 seconds'`, [id]);
        const { rows } = await database_1.default.query(`
      SELECT ts.user_id, ts.user_name, ts.avatar_url
      FROM chat_typing_status ts
      JOIN chat_miembros m ON m.canal_id = ts.canal_id AND m.user_id = ts.user_id
      WHERE ts.canal_id = $1
        AND ts.user_id != $2
        AND ts.updated_at >= NOW() - INTERVAL '6 seconds'
      ORDER BY ts.updated_at DESC
    `, [id, userId]);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function updateTypingStatus(req, res) {
    const userId = req.auth?.userId;
    const userName = resolveAuthDisplayName(req.auth);
    const avatarUrl = req.auth?.sessionClaims?.picture || null;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    const { typing } = req.body;
    try {
        const member = await database_1.default.query(`SELECT 1 FROM chat_miembros WHERE canal_id = $1 AND user_id = $2 LIMIT 1`, [id, userId]);
        if (!member.rows.length)
            return err(res, 'No eres miembro de este canal', 403);
        if (!typing) {
            await database_1.default.query(`DELETE FROM chat_typing_status WHERE canal_id = $1 AND user_id = $2`, [id, userId]);
            return ok(res, { typing: false });
        }
        await database_1.default.query(`
      INSERT INTO chat_typing_status (canal_id, user_id, user_name, avatar_url, updated_at)
      VALUES ($1, $2, $3, $4, NOW())
      ON CONFLICT (canal_id, user_id)
      DO UPDATE SET user_name = EXCLUDED.user_name, avatar_url = EXCLUDED.avatar_url, updated_at = NOW()
    `, [id, userId, userName, avatarUrl]);
        return ok(res, { typing: true });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function editMensaje(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    const { contenido } = req.body;
    if (!contenido?.trim())
        return err(res, 'Contenido vacío', 400);
    try {
        const { rows } = await database_1.default.query(`
      UPDATE chat_mensajes SET contenido = $1, editado = true, updated_at = NOW()
      WHERE id = $2 AND user_id = $3 AND deleted_at IS NULL RETURNING *
    `, [contenido.trim(), id, userId]);
        if (!rows.length)
            return err(res, 'Mensaje no encontrado o sin permisos', 404);
        return ok(res, rows[0]);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function deleteMensaje(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    try {
        await database_1.default.query(`UPDATE chat_mensajes SET deleted_at = NOW() WHERE id = $1 AND user_id = $2`, [id, userId]);
        return ok(res, { id });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function toggleReaccion(req, res) {
    const userId = req.auth?.userId;
    const userName = resolveAuthDisplayName(req.auth);
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    const { emoji } = req.body;
    if (!emoji)
        return err(res, 'Emoji requerido', 400);
    try {
        const existing = await database_1.default.query(`SELECT id FROM chat_reacciones WHERE mensaje_id = $1 AND user_id = $2 AND emoji = $3`, [id, userId, emoji]);
        if (existing.rows.length) {
            await database_1.default.query(`DELETE FROM chat_reacciones WHERE id = $1`, [existing.rows[0].id]);
            return ok(res, { action: 'removed', emoji });
        }
        else {
            await database_1.default.query(`INSERT INTO chat_reacciones (mensaje_id, user_id, user_name, emoji) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING`, [id, userId, userName, emoji]);
            return ok(res, { action: 'added', emoji });
        }
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function getFijados(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    try {
        const { rows } = await database_1.default.query(`
      SELECT f.*, m.contenido, m.user_name, m.avatar_url, m.gif_url, m.image_url, m.tipo, m.created_at AS msg_created_at
      FROM chat_fijados f JOIN chat_mensajes m ON m.id = f.mensaje_id
      WHERE f.canal_id = $1 ORDER BY f.fijado_at DESC
    `, [id]);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function fijarMensaje(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id, mensajeId } = req.params;
    try {
        await database_1.default.query(`INSERT INTO chat_fijados (canal_id, mensaje_id, fijado_por) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, [id, mensajeId, userId]);
        return ok(res, { canal_id: id, mensaje_id: mensajeId });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function desfijarMensaje(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id, mensajeId } = req.params;
    try {
        await database_1.default.query(`DELETE FROM chat_fijados WHERE canal_id = $1 AND mensaje_id = $2`, [id, mensajeId]);
        return ok(res, { id });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function getFavoritos(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { canal_id } = req.query;
    try {
        const params = [userId];
        const canalFilter = canal_id ? 'AND f.canal_id = $2' : '';
        if (canal_id)
            params.push(canal_id);
        const { rows } = await database_1.default.query(`
      SELECT
        f.*,
        m.contenido,
        m.user_id,
        m.user_name,
        m.avatar_url,
        m.gif_url,
        m.image_url,
        m.tipo,
        m.created_at AS msg_created_at
      FROM chat_favoritos f
      JOIN chat_mensajes m ON m.id = f.mensaje_id
      WHERE f.user_id = $1 ${canalFilter}
      ORDER BY f.favorito_at DESC
    `, params);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function toggleFavorito(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    try {
        const existing = await database_1.default.query(`SELECT id FROM chat_favoritos WHERE user_id = $1 AND mensaje_id = $2`, [userId, id]);
        if (existing.rows.length) {
            await database_1.default.query(`DELETE FROM chat_favoritos WHERE user_id = $1 AND mensaje_id = $2`, [userId, id]);
            return ok(res, { action: 'removed', mensaje_id: id });
        }
        const message = await database_1.default.query(`SELECT canal_id FROM chat_mensajes WHERE id = $1 AND deleted_at IS NULL`, [id]);
        if (!message.rows.length)
            return err(res, 'Mensaje no encontrado', 404);
        const canalId = message.rows[0].canal_id;
        await database_1.default.query(`
      INSERT INTO chat_favoritos (user_id, canal_id, mensaje_id)
      VALUES ($1, $2, $3)
      ON CONFLICT (user_id, mensaje_id) DO NOTHING
    `, [userId, canalId, id]);
        return ok(res, { action: 'added', mensaje_id: id, canal_id: canalId });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function buscarMensajes(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { q, canal_id } = req.query;
    if (!q?.trim())
        return ok(res, []);
    try {
        const params = [`%${q.trim()}%`, userId];
        let canalFilter = '';
        if (canal_id) {
            canalFilter = `AND m.canal_id = $3`;
            params.push(canal_id);
        }
        const { rows } = await database_1.default.query(`
      SELECT m.*, c.nombre AS canal_nombre
      FROM chat_mensajes m
      JOIN chat_canales  c  ON c.id = m.canal_id
      JOIN chat_miembros mb ON mb.canal_id = c.id AND mb.user_id = $2
      WHERE m.contenido ILIKE $1 AND m.deleted_at IS NULL ${canalFilter}
      ORDER BY m.created_at DESC LIMIT 50
    `, params);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function getMiembrosGlobal(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    try {
        const { rows } = await database_1.default.query(`
      SELECT DISTINCT ON (user_id) user_id, user_name, avatar_url, role_label, status
      FROM chat_miembros WHERE user_id != $1
      ORDER BY user_id, user_name ASC
    `, [userId]);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function getOrCreateDM(req, res) {
    const userId = req.auth?.userId;
    const userName = resolveAuthDisplayName(req.auth);
    const avatarUrl = req.auth?.sessionClaims?.picture || null;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { target_user_id, target_user_name, target_avatar_url } = req.body;
    if (!target_user_id)
        return err(res, 'target_user_id requerido', 400);
    try {
        const { rows: existing } = await database_1.default.query(`
      SELECT c.* FROM chat_canales c
      WHERE c.tipo = 'directo'
        AND EXISTS (SELECT 1 FROM chat_miembros m WHERE m.canal_id = c.id AND m.user_id = $1)
        AND EXISTS (SELECT 1 FROM chat_miembros m WHERE m.canal_id = c.id AND m.user_id = $2)
      LIMIT 1
    `, [userId, target_user_id]);
        if (existing.length) {
            return ok(res, {
                ...existing[0],
                dm_target_user_id: target_user_id,
                dm_target_user_name: target_user_name || 'Sin nombre',
                dm_target_avatar_url: target_avatar_url || null,
            });
        }
        const nombre = [userName, target_user_name || 'Sin nombre'].sort().join(' · ');
        const { rows } = await database_1.default.query(`INSERT INTO chat_canales (nombre, tipo, created_by) VALUES ($1, 'directo', $2) RETURNING *`, [nombre, userId]);
        const canal = rows[0];
        await database_1.default.query(`
      INSERT INTO chat_miembros (canal_id, user_id, user_name, avatar_url, role, last_read_at)
      VALUES ($1, $2, $3, $4, 'admin', NOW()), ($1, $5, $6, $7, 'miembro', NOW())
      ON CONFLICT (canal_id, user_id) DO NOTHING
    `, [canal.id, userId, userName, avatarUrl, target_user_id, target_user_name || 'Sin nombre', target_avatar_url || null]);
        return ok(res, {
            ...canal,
            dm_target_user_id: target_user_id,
            dm_target_user_name: target_user_name || 'Sin nombre',
            dm_target_avatar_url: target_avatar_url || null,
        }, 201);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function getCanalesDisponibles(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    try {
        const { rows } = await database_1.default.query(`
      SELECT c.id, c.nombre, c.descripcion, c.tipo, c.created_at,
        (SELECT COUNT(*) FROM chat_miembros WHERE canal_id = c.id)::int AS total_miembros
      FROM chat_canales c
      WHERE c.tipo = 'publico' AND c.archivado = false
        AND NOT EXISTS (SELECT 1 FROM chat_miembros m WHERE m.canal_id = c.id AND m.user_id = $1)
      ORDER BY total_miembros DESC, c.nombre ASC
    `, [userId]);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function joinCanal(req, res) {
    const userId = req.auth?.userId;
    const userName = resolveAuthDisplayName(req.auth);
    const avatarUrl = req.auth?.sessionClaims?.picture || null;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    try {
        const { rows: canal } = await database_1.default.query(`SELECT * FROM chat_canales WHERE id = $1`, [id]);
        if (!canal.length)
            return err(res, 'Canal no encontrado', 404);
        if (canal[0].tipo !== 'publico')
            return err(res, 'Solo puedes unirte a canales públicos directamente', 403);
        await database_1.default.query(`
      INSERT INTO chat_miembros (canal_id, user_id, user_name, avatar_url, role, role_label, last_read_at)
      VALUES ($1, $2, $3, $4, 'miembro', 'Miembro', NOW())
      ON CONFLICT (canal_id, user_id) DO UPDATE SET user_name = EXCLUDED.user_name
    `, [id, userId, userName, avatarUrl]);
        return ok(res, { canal_id: id });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function leaveCanal(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    try {
        await database_1.default.query(`DELETE FROM chat_miembros WHERE canal_id = $1 AND user_id = $2`, [id, userId]);
        return ok(res, { canal_id: id });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function addMiembro(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id } = req.params;
    const { target_user_id, target_user_name, target_avatar_url, role_label } = req.body;
    if (!target_user_id)
        return err(res, 'target_user_id requerido', 400);
    try {
        const { rows: self } = await database_1.default.query(`SELECT role FROM chat_miembros WHERE canal_id = $1 AND user_id = $2`, [id, userId]);
        if (!self.length)
            return err(res, 'No eres miembro de este canal', 403);
        await database_1.default.query(`
      INSERT INTO chat_miembros (canal_id, user_id, user_name, avatar_url, role, role_label, last_read_at)
      VALUES ($1, $2, $3, $4, 'miembro', $5, NOW())
      ON CONFLICT (canal_id, user_id) DO UPDATE SET user_name = EXCLUDED.user_name, role_label = COALESCE(EXCLUDED.role_label, chat_miembros.role_label)
    `, [id, target_user_id, target_user_name || 'Sin nombre', target_avatar_url || null, role_label || 'Miembro']);
        return ok(res, { canal_id: id, user_id: target_user_id });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function removeMiembro(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id, uid } = req.params;
    try {
        const { rows: self } = await database_1.default.query(`SELECT role FROM chat_miembros WHERE canal_id = $1 AND user_id = $2`, [id, userId]);
        if (!self.length)
            return err(res, 'No eres miembro', 403);
        if (self[0].role !== 'admin' && uid !== userId)
            return err(res, 'Sin permisos', 403);
        await database_1.default.query(`DELETE FROM chat_miembros WHERE canal_id = $1 AND user_id = $2`, [id, uid]);
        return ok(res, { canal_id: id, user_id: uid });
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function updateMiembroRole(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { id, uid } = req.params;
    const { role, role_label } = req.body;
    try {
        const { rows: self } = await database_1.default.query(`SELECT role FROM chat_miembros WHERE canal_id = $1 AND user_id = $2`, [id, userId]);
        if (!self.length || self[0].role !== 'admin')
            return err(res, 'Sin permisos de admin', 403);
        await database_1.default.query(`UPDATE chat_miembros SET role = COALESCE($1, role), role_label = COALESCE($2, role_label) WHERE canal_id = $3 AND user_id = $4`, [role || null, role_label || null, id, uid]);
        return ok(res, { ok: true });
    }
    catch (e) {
        return err(res, e.message);
    }
}
const backend_1 = require("@clerk/backend");
let _clerk2 = null;
function getClerk2() {
    if (!_clerk2)
        _clerk2 = (0, backend_1.createClerkClient)({ secretKey: process.env.CLERK_SECRET_KEY });
    return _clerk2;
}
const _usersCache = { data: [], exp: 0 };
async function getSystemUsers(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    try {
        if (_usersCache.exp > Date.now())
            return ok(res, _usersCache.data);
        const list = await getClerk2().users.getUserList({ limit: 200 });
        const users = (list.data || []).map((u) => ({
            user_id: u.id,
            user_name: [u.firstName, u.lastName].filter(Boolean).join(' ').trim() || u.emailAddresses?.[0]?.emailAddress || u.username || 'Sin nombre',
            avatar_url: u.imageUrl || null,
            email: u.emailAddresses?.[0]?.emailAddress || null,
            role_label: u.publicMetadata?.role || 'Colaborador',
        }));
        _usersCache.data = users;
        _usersCache.exp = Date.now() + 60000;
        return ok(res, users);
    }
    catch (e) {
        const { rows } = await database_1.default.query(`
      SELECT DISTINCT ON (user_id) user_id, user_name, avatar_url, role_label
      FROM chat_miembros ORDER BY user_id, user_name ASC
    `);
        return ok(res, rows);
    }
}
async function buscarCanalesDisponibles(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    const { q } = req.query;
    try {
        const params = [userId];
        let filter = '';
        if (q?.trim()) {
            filter = `AND (c.nombre ILIKE $2 OR c.descripcion ILIKE $2)`;
            params.push(`%${q.trim()}%`);
        }
        const { rows } = await database_1.default.query(`
      SELECT c.id, c.nombre, c.descripcion, c.tipo, c.created_at,
        (SELECT COUNT(*) FROM chat_miembros WHERE canal_id = c.id)::int AS total_miembros,
        EXISTS(SELECT 1 FROM chat_miembros WHERE canal_id = c.id AND user_id = $1) AS ya_unido
      FROM chat_canales c
      WHERE c.tipo = 'publico' AND c.archivado = false ${filter}
      ORDER BY total_miembros DESC, c.nombre ASC LIMIT 30
    `, params);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
    }
}
async function getUnreadCounts(req, res) {
    const userId = req.auth?.userId;
    if (!userId)
        return err(res, 'No autenticado', 401);
    try {
        const { rows } = await database_1.default.query(`
      SELECT c.id AS canal_id,
        c.tipo,
        ${unreadCountExpr('$1')} AS no_leidos,
        (SELECT dm.user_id FROM chat_miembros dm
          WHERE dm.canal_id = c.id AND dm.user_id != $1
          LIMIT 1
        ) AS dm_target_user_id
      FROM chat_canales c
      JOIN chat_miembros m ON m.canal_id = c.id AND m.user_id = $1
      WHERE c.archivado = false
    `, [userId]);
        return ok(res, rows);
    }
    catch (e) {
        return err(res, e.message);
    }
}
