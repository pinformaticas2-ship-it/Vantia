...
export const createTask = async (req: any, res: Response) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO client_tasks (client_id, titulo, descripcion, plazo, estado, prioridad, expediente, tipo, juzgado, num_proc, created_by)
      ...
      [
        ...
        juzgado?.trim() || null,
        num_proc?.trim() || null,
        await resolveUserName(req.auth?.userId || 'SYSTEM'),
      ]
    );
