...
# 1. Crear script helper en AppData\Roaming\Vantia\
...
param([string]$raw)
# Eliminar prefijo "vantia:"
$b64 = ($raw -replace '^vantia:', '') -replace '-','+' -replace '_','/'
# Padding base64
$pad = (4 - $b64.Length % 4) % 4
if ($pad -lt 4) { $b64 += '=' * $pad }
# Decodificar → URI Office con | literales
$uri = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($b64))
# Abrir con el manejador de protocolo del SO (ms-word:, ms-excel:, etc.)
Start-Process $uri
'@

# 2. Registrar protocolo vantia: en HKCU (sin admin)
